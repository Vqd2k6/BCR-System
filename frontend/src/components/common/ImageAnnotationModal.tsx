...
export const ImageAnnotationModal: React.FC<ImageAnnotationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-2 sm:p-4">
      <div className="bg-white border border-slate-200 text-slate-900 rounded-2xl w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        {/* Toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 bg-slate-50/80 border-b border-slate-200 text-xs">
          {/* Tools */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 transition-all ${
                activeTool === 'ARROW' ? 'bg-sky-600 text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 transition-all ${
                activeTool === 'PEN' ? 'bg-sky-600 text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg font-bold flex items-center gap-1 transition-all ${
                activeTool === 'CIRCLE' ? 'bg-sky-600 text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg font-bold flex items-center gap-1 transition-all ${
                activeTool === 'RECT' ? 'bg-sky-600 text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100'
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg font-bold flex items-center gap-1 transition-all ${
                activeTool === 'TEXT' ? 'bg-sky-600 text-white shadow-sm' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100'
              }`}
            >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1 bg-white p-1 rounded-lg border border-slate-200 shadow-2xs">
              {COLORS.map((c) => (
                <button
                  ...
                  title={c.label}
                  className={`w-5 h-5 rounded-full transition-transform border border-slate-300 ${
                    selectedColor === c.value ? 'ring-2 ring-sky-500 scale-110' : 'opacity-80 hover:opacity-100'
                  }`}
                />
              ))}
            </div>
            ...
            <button
              ...
              className="p-1.5 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 disabled:opacity-40 rounded-lg shadow-2xs"
              title="Hoàn tác"
            >
            ...
            <button
              ...
              onClick={handleClear}
              disabled={annotations.length === 0}
              className="p-1.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 disabled:opacity-40 rounded-lg"
              title="Xóa tất cả"
            >
        ...
        {activeTool === 'TEXT' && (
          <div className="flex items-center gap-2 px-4 py-1.5 bg-sky-50/70 border-b border-sky-100 text-xs">
            <span className="text-sky-900 font-semibold">Chữ cần chèn:</span>
            <input
              type="text"
              value={customText}
              onChange={(e) => setCustomText(e.target.value)}
              placeholder="Nhập nội dung rồi chạm vào ảnh để đặt..."
              className="bg-white border border-slate-300 rounded px-2 py-0.5 text-xs text-slate-900 flex-1 max-w-sm focus:ring-1 focus:ring-sky-500"
            />
          </div>
        )}

        {/* Canvas Workspace */}
        <div className="flex-1 bg-slate-100 flex items-center justify-center p-3 overflow-auto select-none min-h-[300px]">
          {imageObjRef.current && (
            <canvas
              ref={canvasRef}
              ...
              className="max-h-[60vh] max-w-full object-contain cursor-crosshair border border-slate-300 rounded-lg shadow-md bg-white touch-none"
            />
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 transition-colors"
            >
              Hủy bỏ
            </button>
            <button
